...
plot_n_interactions <- function() {
  ...
  rc_order <- names(CELL_TYPE_ABBREVIATIONS_PTs)
...
ggsave_publication("heatmap", plot = p, width = 8, height = 7,type="png")
...
for (i in 1:length(pathways.show.all)) {
  ...
  g2<-plotGeneExpression(cellchat,signaling=pathways.show.all[i], enriched.only = TRUE,angle.x=45)
  g2<-g2+plot_annotation(paste(pathways.show.all[i],"signaling pathway"),theme=theme(plot.title=element_text(hjust=0.5,size=18)))
  #g2<- g2 + ggtitle(paste("Expression of enriched genes in ",pathways.show.all[i],"signaling"))
  ggsave(filename=paste0(pathways.show.all[i], "_GeneExpression.pdf"), plot=g2, width = 8, height = 10, units = 'in', dpi = 300)
